...
const CardItem = ({ publicacao }) => {
  ...
  const listaComentario = publicacao.lista_comentarios
    ? Object.values(publicacao.lista_comentarios)
    : [];
...
const Card = ({ publicacoes }) => {
    ...
    const publicacoesIvertidas = [...publicacoes].reverse();

  return (
    <section className="cardsContainer">
      {publicacoesIvertidas.map((publicacao, index) => (
        <CardItem key={index} publicacao={publicacao} />
      ))}
    </section>
  );
};
